  ...
  Future<void> _fetchExplanations() async {
    ...
    for (var question in widget.incorrectQuestions) {
      final response = await http.post(
        Uri.parse('http://192.168.29.108:5000/generate_explanation'), // Adjust the host/port if needed
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 10),
            ...question['options'].map<Widget>((opt) => Text(opt, style: TextStyle(fontSize: 16))),
            SizedBox(height: 10),
            ...
            Row(
              ...
              children: [
                if (_currentIndex > 0)
                  ElevatedButton(onPressed: () => setState(() => _currentIndex--), child: Text("Previous")),
                if (_currentIndex < widget.incorrectQuestions.length - 1)
                  ElevatedButton(onPressed: () => setState(() => _currentIndex++), child: Text("Next")),
              ],
            ),
          ],
        ),
